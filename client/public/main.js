...
(() => {
  ...
  const renderPolicyDrawer = async policyId => {
    ...
    try {
      ...
      const rulesHtml = rules.length ? rules.map(rule => `<div class="activity-item" data-rule-id="${esc(rule.id)}"><div><b>${esc(rule.name)}</b><span>${esc(rule.action)}${rule.resource ? ` · ${esc(rule.resource)}` : ''} · ${esc(rule.effect)} · ${esc(rule.risk_level)}</span><small>${rule.approval_required ? 'Approval required' : 'No approval required'} · ${esc(JSON.stringify(rule.conditions || {}))}</small></div><div><button class="row-action policy-rule-delete" type="button" data-rule-id="${esc(rule.id)}">Delete</button></div></div>`).join('') : '<div class="app-empty-state"><strong>No rules yet</strong><span>Add a rule to define this policy’s stored governance data.</span></div>';
  ...
  document.addEventListener('click', async event => {
    ...
    try {
      target.disabled = true;
      target.textContent = 'CREATING…';
      await auth().createPolicy({ name: policyFormValue('policy-name'), description: policyFormValue('policy-description'), status: policyFormValue('policy-status') || 'draft', rules: readPolicyRuleForm() });
      document.querySelector('#app-modal').hidden = true;
