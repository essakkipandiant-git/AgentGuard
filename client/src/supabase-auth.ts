...
export const agentGuardAuth = {
  ...
  async getWorkspaceData() {
    ...
    if (!session?.user) return { workspaceId: null, agents: [], policies: [], approvals: [], auditLogs: [], activity: [] };
    const membership = await supabase.from('workspace_members').select('workspace_id,role').eq('user_id', session.user.id).limit(1).maybeSingle();
    if (membership.error) throw membership.error;
    const workspaceId = membership.data?.workspace_id ?? null;
    if (!workspaceId) return { workspaceId: null, agents: [], policies: [], approvals: [], auditLogs: [], activity: [] };
    ...
    return { workspaceId, agents: agents.data ?? [], policies: policies.data ?? [], approvals: approvals.data ?? [], auditLogs: auditLogs.data ?? [], activity: activity.data ?? [] };
  },
...
const initializeAuth = () => {
  ...
  if (window.location.pathname === '/reset-password') {
    document.body.innerHTML = `<main class="signin-tab" style="display:grid"><section class="signin-panel"><div class="signin-brand"><img src="/assets/logo.webp" alt="AgentGuard mark" width="42" height="42" /><span>AGENTGUARD<small>CONTROL PLANE ACCESS</small></span></div><div class="signin-kicker">AUTHENTICATION / PASSWORD RESET</div><h2>Set a new<br><em>password.</em></h2><p class="signin-intro">Choose a new password for your AgentGuard control plane.</p><form class="signin-form" id="reset-password-form"><label>NEW PASSWORD<input name="password" type="password" minlength="6" required placeholder="At least 6 characters"></label><p class="signin-error" id="reset-password-error" hidden></p><button class="signin-submit" type="submit">Update password <i class="fa-solid fa-arrow-right"></i></button></form><div class="signin-status"><i></i> SECURE AUTHENTICATION / SESSION ENCRYPTED</div></section></main>`;
    document.querySelector('#reset-password-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget as HTMLFormElement; const error = document.querySelector('#reset-password-error') as HTMLElement; const button = form.querySelector('button') as HTMLButtonElement; const password = (form.elements.namedItem('password') as HTMLInputElement).value; button.disabled = true; button.textContent = 'UPDATING...'; const { error: updateError } = await supabase!.auth.updateUser({ password }); if (updateError) { error.textContent = updateError.message; error.hidden = false; button.disabled = false; button.textContent = 'Update password'; return; } button.textContent = 'PASSWORD UPDATED'; setTimeout(() => window.location.replace('/dashboard'), 700); });
    return;
  }
  supabase.auth.onAuthStateChange((_event, session) => {
    window.dispatchEvent(new CustomEvent('agentguard:auth-state', { detail: session }));
    const protectedRoute = ['/dashboard', '/app', '/agents', '/policies', '/activity', '/security'].some(path => window.location.pathname === path || window.location.pathname.startsWith(`${path}/`));
    if (protectedRoute && !session) window.location.replace('/?signin=1');
  });
...
if (document.readyState === 'loading') window.addEventListener('DOMContentLoaded', initializeAuth, { once: true });
else initializeAuth();
